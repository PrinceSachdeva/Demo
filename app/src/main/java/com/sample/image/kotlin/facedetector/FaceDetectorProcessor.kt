...
package com.sample.image.kotlin.facedetector
...
import com.sample.image.kotlin.VisionProcessorBase
import java.util.Locale

/** Face Detector Demo.  */
class FaceDetectorProcessor(context: Context, detectorOptions: FaceDetectorOptions?,  val listener  : OnFaceSelectedListener) :
    ...
    override fun onSuccess(faces: List<Face>, graphicOverlay: GraphicOverlay, bitmap: Bitmap) {
        if (faces != null && faces.size > 0)
            for (face in faces) {
                listener.onFaceSelected(face, bitmap)
                graphicOverlay.add(FaceGraphic(graphicOverlay, face))
                logExtrasForTesting(face)
                break
            } else {
            listener.onFaceSelectionError()
            Log.e(TAG, "Face detection failed zero responses")
        }
    }
    ...
    companion object {
        ...
        private fun logExtrasForTesting(face: Face?) {
            if (face != null) {
                Log.v(
                    MANUAL_TESTING_LOG,
                    "face allContours: " + face.allContours.toString()
                )
                Log.v(
                    MANUAL_TESTING_LOG,
